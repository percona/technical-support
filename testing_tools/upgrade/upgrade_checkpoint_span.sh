...
# In particular, this upgrade test verifies that a dirty upgrade will work
# with a workload that leaves the bin log with a transaction that has spanned
# a TokuDB checkpoint.  This script requires checkpoint_client_1.sql and
# checkpoint_client_2.sql as well as verify_checkpoint.sql to run.

if [ $# -eq 0 ]; then
  echo "usage: upgrade_checkpoint_span.sh <tarball_1.tar.gz> <tarball_2.tar.gz>"
  exit 1
fi
...
echo
echo "Running two client connections against the DB to induce a transaction commit that spans a checkpoint"
echo
bin/mysql --user=root --socket=/tmp/dirty_upgrade_1.sock test < ${workingdir}/checkpoint_client_1.sql &
my_client_pid=$!
sleep 5
bin/mysql --user=root --socket=/tmp/dirty_upgrade_1.sock test < ${workingdir}/checkpoint_client_2.sql
wait ${my_client_pid}
...
echo
${workingdir}/tdb_logprint < data/log*.tokulog* | tail -20
echo
echo "######################################"
echo
...
if [ "${my_dirty_upgraded_mysqld_pid}" == "${my_dirty_upgraded_mysqld_verification}" ]; then
    ...
    echo
    bin/mysql --user=root --socket=/tmp/dirty_upgrade_2.sock < ${workingdir}/verify_checkpoint.sql  
    echo
    echo "CHECKSUM from the verify script should be 204."
...
echo "Running two client connections against the DB to induce a transaction commit that spans a checkpoint"
echo
bin/mysql --user=root --socket=/tmp/clean_upgrade_1.sock test < ${workingdir}/checkpoint_client_1.sql &
my_client_pid=$!
sleep 5
bin/mysql --user=root --socket=/tmp/clean_upgrade_1.sock test < ${workingdir}/checkpoint_client_2.sql
wait ${my_client_pid}
...
echo "Verifying the data in all tables"
echo
bin/mysql --user=root --socket=/tmp/clean_upgrade_2.sock < ${workingdir}/verify_checkpoint.sql
echo
echo "CHECKSUM from the verify script should be 204"
